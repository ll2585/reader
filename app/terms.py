...
class Terms:
				...
				if(line != ''):
					...
					if cnt >= 1:
						if cnt >= 5:
							...
							self.add_term(Term(cols[0].strip(), Lemma(cols[0].strip(), cols[1].strip(),
							...
							self.add_term(Term(cols[0].strip(), Lemma(cols[0].strip(), '?',
									'', '', '', '', '', '', 1)))
				self.dirty = False
				r = True
		return r
	...
	def isSaveTermsToDBOK(self):
		...
		try:
			...
			print(self.edited_notes)
			if(len(new_lemmas) > 0):
				conn.executemany('INSERT INTO lemmas(language, lemma, definition, translation, priorSentence, sentenceCLOZE, sentence, followingSentence, source, status) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', new_lemmas)
			if(len(new_terms) > 0):
				conn.executemany('INSERT INTO terms(language, term, root, rootID) values (?, ?, ?, ?)', new_terms)
			if(len(updated_lemmas) > 0):
				conn.executemany('UPDATE lemmas SET definition = ?, translation = ?, priorSentence = ?, sentenceCLOZE = ? , sentence = ?, followingSentence = ?, source = ?, status = ? WHERE id = ?', updated_lemmas)
			if(len(new_joins) > 0):
				conn.executemany('INSERT INTO lemmas_terms(lemma_id, term_id, language, notes) values (?, ?, ?, ?)', new_joins)
			...
			if (len(self.edited_notes) > 0):
				for r in self.edited_notes:
					conn.execute('UPDATE lemmas_terms SET notes=:notes WHERE lemma_id=:lemma_id AND term_id=:term_id', r)
			conn.commit()
			conn.close()
			self.dirty = False
			r = True
...
class Term():
	...
	def to_sql(self):
		return (gui.application.getLanguage().getLangName(), self.word,self.get_first_possible_lemma().word,self.get_first_possible_lemma().id)
	...
	def display_with_status_html(self):
		...
		if len(self.possible_lemmas) > 0:
			...
			result = utilities.escapeHTML('{0}<br>lemma: {1}<br>{2}'.format(self.word, lemma_text, utilities.escapeHTML('{0}<br>{1} — {2}'.format(definition, translation, status))))
		else:
			lemma_text = "(No Lemma)"
			translation = ""
			definition = ""
	...
	def __str__(self):
		return '%s%s%s%s%s%s%s%s%s' %(self.word, constants.TAB, self.get_first_possible_lemma().definition, constants.TAB, self.get_first_possible_lemma().translation, constants.TAB, self.get_first_possible_lemma().sentence, constants.TAB, str(self.get_status().getStatusCode()))
...
class Lemma:
	...
	def new_empty_lemma(self):
		return Lemma(-1,'','','', self.priorSentence, self.sentenceCLOZE, self.sentence, self.followingSentence, self.source, 1)
