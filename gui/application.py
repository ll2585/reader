...
class StartFrame(QtGui.QMainWindow):
    ...
    def changeDirectory(self):
        dir = utilities.selectDirectory(self, "Select %s data directory..." % (constants.SHORT_NAME),
                                        os.path.expanduser('~'))
    ...
    def readStudy(self):
        ...
        if (lang):
            ...
            if not terms.is_load_terms_from_db_ok(dbPath):
                ...
                if os.path.exists(fTermFile) or os.path.exists(fLangSettingsFile):
                    ...
                    if l2 >= 0:
                        ...
                        if l1code != "" and l2code != "":
                            lang.putPref(language.KEY_dictionary_url_1,
                                         "http://translate.google.com/?ie=UTF-8&sl=%s&t;=%s&text=###" % (
                                         l2code, l1code))
                            lang.putPref(language.KEY_dictionary_url_2,
                                         "http://glosbe.com/%s/%s/###" % (l2code, l1code2))
                            if ld2.isTtsAvailable():
                                lang.putPref(language.KEY_dictionary_url_3,
                                             "http://translate.google.com/translate_tts?ie=UTF-8&tl=%s&q=###" % l2code)
                        lang.saveFile()
                    else:
                        if l1code != "":
                            lang.putPref(language.KEY_dictionary_url_1,
                                         "http://translate.google.com/?ie=UTF-8&sl=auto&tl=%s&text=###" % l1code)
                            lang.saveFile()
                    setLanguage(lang)
                    ...
                    if (ok1 and ok2 and ok3):
                        utilities.showInfoMessage("Language '%s' successfully created.\n\n"
                                                  ...
                                                  "%s." % (
                                                  newLang, os.path.abspath(fnewLangDir), os.path.abspath(fTermFile)))
